...
class CommonFunction {
    ...
    func navigateBack(_ vc : AnyObject ,_ fromSb : String ,_ toSb : String ) {
            let storyBoard = UIStoryboard(name: fromSb, bundle: nil)
           _ = storyBoard.instantiateViewController(withIdentifier: toSb)
...
extension CommonFunction {
    ...
    func animatedDelayLabel(_ statusLabel : UILabel) {
    ...
    func sessionCreate(_ token : String) {
        keyChainAccess["token"] = token
        let status = "Login Successfully"
        UserDefaults.standard.set(status, forKey: "status")
    }
    func sessionExpire() {
        keyChainAccess["token"] = nil
